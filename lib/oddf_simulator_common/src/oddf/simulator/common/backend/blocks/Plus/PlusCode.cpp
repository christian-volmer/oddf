...
{
	size_t inputsCount = inputs.GetSize();
	...
	for (size_t i = 0; i < inputsCount; ++i) {
		...
		context.BindInputReference(input.GetIndex(), instruction->m_operands[i].m_input);
	}
	...
	IListView<SimulatorBlockInput const &> const &inputs)
{
	...
	size_t inputsPerOutput = inputs.GetSize() / outputsCount;

	for (size_t i = 0; i < outputsCount; ++i)
		EmitPlusInstruction(context, outputs.Item(i), *inputs.Span(inputsPerOutput * i, inputsPerOutput * (i + 1) - 1));
}
